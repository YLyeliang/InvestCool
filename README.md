...
- NDX 执行票据，通过 `/api/risk/execution-ticket` 把投前检查、仓位 sizing、触发线、Playbook 和保护覆盖压成观察单、修复加仓、确认加仓和失效降档四步交易台票据
- NDX 风险登记簿，通过 `/api/risk/risk-register` 把预警、触发线、风险回报、期权、财报、集中度、流动性和数据质量压成概率、影响、触发条件、缓释动作与责任模块
...
- NDX 对冲方案簿，通过 `/api/risk/hedge-book` 把对冲覆盖、期权隐含区间、偏斜、Gamma、波动风险溢价、触发线和 Playbook 压成 put spread、collar、现金缓冲和触发降档等保护结构菜单
