    ...
    <div className="analysis-page space-y-10 pb-20">
      <header className="page-header space-y-4">
        <h2 className="text-4xl font-black tracking-tighter text-[var(--text-primary)]">
          深度投研<span className="text-blue-500">.</span>
        </h2>
      ...
      {/* 分类导航 - 现代化胶囊设计 */}
      <nav className="category-nav flex items-center gap-2 overflow-x-auto pb-4 no-scrollbar sticky top-[var(--header-height)] z-20 bg-[var(--bg-color)]/80 backdrop-blur-md -mx-4 px-4 py-2">
        {categories.map((cat) => (
          <button
            key={cat}
            onClick={() => setActiveCategory(cat)}
            className={cn(
              "px-5 py-2 rounded-full text-xs font-black transition-all whitespace-nowrap border uppercase tracking-wider",
              activeCategory === cat
                ? "bg-slate-900 dark:bg-white text-white dark:text-black border-transparent shadow-lg"
                : "bg-white dark:bg-slate-900 text-slate-400 border-[var(--border-color)] hover:border-blue-500 hover:text-blue-500"
            )}
          >
            {cat}
          </button>
        ...
        <AnimatePresence mode="popLayout">
          {filteredArticles.length > 0 ? (
            ...
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="py-32 text-center border-2 border-dashed border-[var(--border-color)] rounded-[2.5rem]"
            >
