...
export default function HKQuickGuidePage() {
  return (
    <div className="max-w-4xl pb-20">
      <Link href="/hk-account" className="inline-flex items-center gap-2 text-sm font-bold text-emerald-500 mb-8">
        <Icon name="lucide:arrow-left" className="size-4" />
        返回香港开户指南
      </Link>

      <header className="mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-900/30 text-emerald-500 text-[0.75rem] font-black uppercase tracking-widest mb-5">
        ...
        <div className="space-y-3">
          {checklist.map((item, index) => (
            <div key={item} className="flex items-start gap-3 text-sm font-medium text-[var(--text-secondary)]">
              <span className="size-6 rounded-lg bg-emerald-500 text-white flex items-center justify-center text-xs font-black shrink-0">
                {index + 1}
              </span>
              <span>{item}</span>
            </div>
        ...
        <ul className="space-y-3">
          {avoidList.map((item) => (
            <li key={item} className="flex items-start gap-3 text-sm font-medium text-[var(--text-secondary)]">
              <Icon name="lucide:alert-triangle" className="size-5 text-amber-500 shrink-0 mt-0.5" />
              <span>{item}</span>
            </li>
          ))}
        </ul>
