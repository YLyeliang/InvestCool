"use client";

import React, { useEffect, useState } from "react";
import { Icon } from "@/components/ui/Icon";
import { cn } from "@/lib/utils";

type ColorKey = "green" | "blue" | "amber" | "red";

interface ScenarioRange {
  low: number;
  high: number;
  label: string;
}

interface ProbabilityScenario {
  key: string;
  name: string;
  category: string;
  color: ColorKey;
  probability_pct: number;
  midpoint_move: number;
  expected_contribution: number;
  ndx_range: ScenarioRange;
  trigger_count: number;
  response: string;
  rationale: string;
}

interface ScenarioMapPayload {
  as_of: string;
  index: number;
  risk_score: number;
  posture: string;
  posture_color: ColorKey;
  expected_move: number;
  downside_probability: number;
  upside_probability: number;
  probability_weighted_low: number;
  probability_weighted_high: number;
  dominant_scenario: string;
  worst_scenario: string;
  best_scenario: string;
  stress_downside: number;
  data_coverage: string;
  summary: string;
  scenarios: ProbabilityScenario[];
  controls: string[];
  methodology: string;
}

const colorMap = {
  green: {
    text: "text-emerald-700",
    bg: "bg-emerald-500",
    soft: "bg-emerald-500/10",
    border: "border-emerald-500/25",
  },
  blue: {
    text: "text-blue-700",
    bg: "bg-blue-500",
    soft: "bg-blue-500/10",
    border: "border-blue-500/25",
  },
  amber: {
    text: "text-amber-700",
    bg: "bg-amber-500",
    soft: "bg-amber-500/10",
    border: "border-amber-500/25",
  },
  red: {
    text: "text-red-700",
    bg: "bg-red-500",
    soft: "bg-red-500/10",
    border: "border-red-500/25",
  },
};

const formatDateTime = (dateStr: string) => {
  return new Date(dateStr).toLocaleString("zh-CN", {
    month: "2-digit",
    day: "2-digit",
    hour: "2-digit",
    minute: "2-digit",
  });
};

const formatIndex = (value: number) => value.toLocaleString("zh-CN", { maximumFractionDigits: 2 });
const formatSignedPct = (value: number) => `${value >= 0 ? "+" : ""}${value.toFixed(2)}%`;

export const NDXScenarioMapPanel = () => {
  const [payload, setPayload] = useState<ScenarioMapPayload | null>(null);
  const [pending, setPending] = useState(true);

  useEffect(() => {
    let cancelled = false;
    let retryTimer: ReturnType<typeof setTimeout> | null = null;

    const fetchScenarioMap = async (attempt = 0) => {
      if (attempt === 0) setPending(true);
      try {
        const res = await fetch("/api/risk/scenario-map");
        const data = await res.json();
        if (cancelled) return;
        if (res.ok && !data.error) {
          setPayload(data);
          setPending(false);
          return;
        }
        if (attempt < 4) {
          retryTimer = setTimeout(() => void fetchScenarioMap(attempt + 1), 4000);
          return;
        }
        setPayload(null);
      } catch (e) {
        console.error("Failed to fetch NDX scenario probability map:", e);
        if (!cancelled && attempt < 4) {
          retryTimer = setTimeout(() => void fetchScenarioMap(attempt + 1), 4000);
          return;
        }
        if (!cancelled) setPayload(null);
      } finally {
        if (!cancelled && attempt >= 4) setPending(false);
      }
    };

    void fetchScenarioMap();

    return () => {
      cancelled = true;
      if (retryTimer) clearTimeout(retryTimer);
    };
  }, []);

  if (pending && !payload) {
    return (
      <div className="rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] p-6 shadow-sm">
        <div className="h-5 w-52 rounded bg-[var(--section-bg)] animate-pulse mb-5" />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="h-40 rounded-lg bg-[var(--section-bg)] animate-pulse" />
          ))}
        </div>
      </div>
    );
  }

  if (!payload) {
    return (
      <div className="rounded-lg border-2 border-dashed border-[var(--border-color)] py-10 text-center text-[var(--text-tertiary)]">
        <Icon name="workflow" size={36} className="mx-auto mb-3 opacity-30" />
        <p className="text-sm font-semibold">情景概率图正在初始化...</p>
      </div>
    );
  }

  const postureColor = colorMap[payload.posture_color] ?? colorMap.blue;
  const expectedWidth = `${Math.max(4, Math.min(100, 50 + payload.expected_move * 8))}%`;
  const upsideWidth = `${Math.max(4, Math.min(100, payload.upside_probability))}%`;
  const downsideWidth = `${Math.max(4, Math.min(100, payload.downside_probability))}%`;

  return (
    <div className="space-y-5">
      <div className={cn("rounded-lg border p-5 shadow-sm", postureColor.soft, postureColor.border)}>
        <div className="grid grid-cols-1 lg:grid-cols-[190px_1fr_210px] gap-5 items-center">
          <div>
            <div className="text-xs font-black uppercase text-[var(--text-tertiary)] mb-1">
              Scenario Probability
            </div>
            <div className={cn("text-3xl font-black leading-none", postureColor.text)}>
              {payload.posture}
            </div>
            <div className="text-sm font-black text-[var(--text-primary)] mt-2">
              E[Move] {formatSignedPct(payload.expected_move)}
            </div>
          </div>

          <div>
            <p className="text-lg leading-8 font-bold text-[var(--text-primary)] m-0">
              {payload.summary}
            </p>
            <p className="text-sm leading-6 text-[var(--text-secondary)] mt-2 mb-0">
              {payload.methodology}
            </p>
          </div>

          <div className="text-sm font-semibold text-[var(--text-secondary)] lg:text-right">
            <div>{payload.data_coverage}</div>
            <div>NDX {formatIndex(payload.index)} · 风险分 {payload.risk_score.toFixed(1)}</div>
            <div className="text-xs text-[var(--text-tertiary)] mt-1">
              更新 {formatDateTime(payload.as_of)}
            </div>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5">
          <Gauge label="概率加权涨跌" value={formatSignedPct(payload.expected_move)} width={expectedWidth} color={postureColor.bg} />
          <Gauge label="上行概率" value={`${payload.upside_probability.toFixed(1)}%`} width={upsideWidth} color="bg-emerald-500" />
          <Gauge label="下行概率" value={`${payload.downside_probability.toFixed(1)}%`} width={downsideWidth} color="bg-red-500" />
        </div>
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-4">
        <div className="rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3 mb-4">
            <h4 className="text-sm font-black text-[var(--text-primary)]">情景概率贡献</h4>
            <span className="text-xs font-black text-[var(--text-tertiary)]">按概率排序</span>
          </div>
          <div className="space-y-3">
            {payload.scenarios.map((scenario) => {
              const color = colorMap[scenario.color] ?? colorMap.blue;
              const probabilityWidth = `${Math.max(4, Math.min(100, scenario.probability_pct))}%`;
              return (
                <article key={scenario.key} className="rounded-lg bg-[var(--section-bg)] p-4">
                  <div className="grid grid-cols-1 md:grid-cols-[1fr_140px] gap-3">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className={cn("size-2 rounded-full", color.bg)} />
                        <span className="text-[11px] font-black uppercase text-[var(--text-tertiary)]">
                          {scenario.category}
                        </span>
                      </div>
                      <h5 className="text-base font-black text-[var(--text-primary)] mb-2">
                        {scenario.name}
                      </h5>
                      <p className="text-sm leading-6 text-[var(--text-secondary)] m-0">
                        {scenario.rationale}
                      </p>
                    </div>
                    <div className="md:text-right">
                      <div className={cn("text-2xl font-black", color.text)}>
                        {scenario.probability_pct.toFixed(1)}%
                      </div>
                      <div className="text-xs font-bold text-[var(--text-tertiary)]">
                        贡献 {formatSignedPct(scenario.expected_contribution)}
                      </div>
                    </div>
                  </div>
                  <div className="mt-4">
                    <div className="flex items-center justify-between text-[11px] font-black uppercase text-[var(--text-tertiary)] mb-2">
                      <span>{scenario.ndx_range.label}</span>
                      <span>{formatSignedPct(scenario.midpoint_move)}</span>
                    </div>
                    <div className="h-2 rounded-full bg-[var(--card-bg)] overflow-hidden">
                      <div className={cn("h-full rounded-full", color.bg)} style={{ width: probabilityWidth }} />
                    </div>
                  </div>
                </article>
              );
            })}
          </div>
        </div>

        <div className="space-y-4">
          <InfoCard title="概率加权区间" icon="brackets" items={[
            `加权下沿 ${formatIndex(payload.probability_weighted_low)}`,
            `加权上沿 ${formatIndex(payload.probability_weighted_high)}`,
            `压力回撤 -${payload.stress_downside.toFixed(1)}%`,
          ]} />
          <InfoCard title="关键情景" icon="route" items={[
            `主导：${payload.dominant_scenario}`,
            `下行：${payload.worst_scenario}`,
            `上行：${payload.best_scenario}`,
          ]} />
          <InfoCard title="执行约束" icon="clipboard-check" items={payload.controls} />
        </div>
      </div>
    </div>
  );
};

const Gauge = ({ label, value, width, color }: { label: string; value: string; width: string; color: string }) => (
  <div>
    <div className="flex justify-between text-[11px] font-black uppercase text-[var(--text-tertiary)] mb-2">
      <span>{label}</span>
      <span>{value}</span>
    </div>
    <div className="h-2 rounded-full bg-[var(--card-bg)]/70 overflow-hidden">
      <div className={cn("h-full rounded-full", color)} style={{ width }} />
    </div>
  </div>
);

const InfoCard = ({ title, icon, items }: { title: string; icon: string; items: string[] }) => (
  <div className="rounded-lg border border-[var(--border-color)] bg-[var(--section-bg)] p-4">
    <div className="flex items-center gap-2 mb-3">
      <Icon name={icon} size={16} className="text-[var(--accent-color)]" />
      <h4 className="text-sm font-black text-[var(--text-primary)]">{title}</h4>
    </div>
    <div className="space-y-2">
      {items.map((item) => (
        <div key={item} className="flex gap-2 text-sm leading-6 text-[var(--text-secondary)]">
          <Icon name="circle-dot" size={14} className="mt-1 shrink-0 text-[var(--accent-color)]" />
          <span>{item}</span>
        </div>
      ))}
    </div>
  </div>
);
